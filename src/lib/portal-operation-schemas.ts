...
export const portalOperationActionSchema = z.discriminatedUnion("action", [
  ...
  z.object({
    ...
    person_type: z.enum(["crew", "visitor"]).default("crew"),
    organisation: z.string().trim().max(200).nullable().optional(),
    movement_type: z.enum(["on_signing", "off_signing", "visitor", "other"]).default("other"),
  ...
  z.object({
    ...
    name: z.string().trim().min(1).max(160),
    email: z.string().email().nullable().optional(),
    person_type: z.enum(["crew", "visitor"]),
    organisation: z.string().trim().max(200).nullable().optional(),
    movement_type: z.enum(["on_signing", "off_signing", "visitor", "other"]),
    flight_information: z.string().trim().max(300).nullable().optional(),
