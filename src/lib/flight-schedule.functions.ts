...
const draftRecordSchema = z.object({
  ...
  origin: z.string().trim().regex(/^[A-Z]{3}$/),
  destination: z.string().trim().regex(/^[A-Z]{3}$/),
...
function toIsoDate(value: string) {
  ...
  const year = Number(yearValue ?? (localYear && Number(localYear) < 100 ? `20${localYear}` : localYear));
...
export const getFlightScheduleOverview = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const [{ data: active, error: activeError }, { data: imports, error: importsError }, { data: drafts, error: draftsError }] =
      await Promise.all([
        ...
        (sb as any)
          .from("flight_schedule_versions")
          .select("id, name, status, coverage_start, coverage_end, created_by, created_at")
          .eq("status", "draft")
          .order("created_at", { ascending: false })
          .limit(25),
      ]);
    ...
    return { active, imports: imports ?? [], drafts: drafts ?? [] };
  });
