...
export const getMyCompany = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    return await lookupCompanyForUser(
      supabaseAdmin,
      context.userId,
      "id, name, status, access_end, require_client_company, custom_link, logo_url, advert_url, advert_link, advert_caption, advert_enabled, referral_code, operations_phone, default_departure_pickup_offset_minutes, default_arrival_pickup_offset_minutes, minimum_connection_buffer_minutes",
    );
  });

const pickupOffsetMinutesInput = z.number().int().min(0).max(24 * 60);
const connectionBufferMinutesInput = z.number().int().min(0).max(24 * 60);

/** Company defaults used when a coordinator links a scheduled transport. */
export const updateMyOperationalPickupOffsets = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: unknown) => z.object({
    departure_offset_minutes: pickupOffsetMinutesInput,
    arrival_offset_minutes: pickupOffsetMinutesInput,
  }).parse(input))
  .handler(async ({ data, context }) => {
    ...
    const { data: updated, error } = await supabaseAdmin
      .from("companies")
      .update({
        default_departure_pickup_offset_minutes: data.departure_offset_minutes,
        default_arrival_pickup_offset_minutes: data.arrival_offset_minutes,
      } as any)
      .eq("id", company.id)
      .select("default_departure_pickup_offset_minutes, default_arrival_pickup_offset_minutes")
      .single();
    if (error) throw new Error(error.message);
    return updated;
  });

export const updateMyMinimumConnectionBuffer = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: unknown) => z.object({ minimum_connection_buffer_minutes: connectionBufferMinutesInput }).parse(input))
  .handler(async ({ data, context }) => {
    const company = await resolveCompany(context);
    const supabaseAdmin = await getAdminClient();
    const { data: updated, error } = await (supabaseAdmin as any).from("companies")
      .update({ minimum_connection_buffer_minutes: data.minimum_connection_buffer_minutes })
      .eq("id", company.id)
      .select("minimum_connection_buffer_minutes")
      .single();
    if (error) throw new Error(error.message);
    return updated as { minimum_connection_buffer_minutes: number };
  });
...
async function assertActiveFlightScheduleRecord(supabaseAdmin: any, recordId: string) {
  // Generated types are refreshed by the Lovable-owned project after migrations.
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const schedules = supabaseAdmin as any;
  const { data, error } = await schedules
    .from("flight_schedule_records")
    .select("id, flight_schedule_versions!inner(status)")
    .eq("id", recordId)
    .eq("flight_schedule_versions.status", "active")
    .maybeSingle();
  if (error) throw new Error(error.message);
  if (!data) throw new Error("Choose a flight from the active schedule.");
}

async function assertActiveDepartureFlightScheduleRecord(supabaseAdmin: any, recordId: string) {
  const schedules = supabaseAdmin as any;
  const { data, error } = await schedules
    .from("flight_schedule_records")
    .select("id, flight_schedule_versions!inner(status)")
...
async function resolveScheduledTransportPickup(
  ...
  if (!scheduleRecordId) {
    return {
      date: fallbackDate,
      time: fallbackTime,
      pickup_at: makePickupIso(fallbackDate, fallbackTime),
      offset_minutes: null,
    };
  }
  ...
  const isDeparture = record.direction === "departure";
  const defaultOffset = isDeparture
...
export const searchActiveFlightScheduleRecords = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  ...
  .handler(async ({ data }) => {
    ...
    // Generated types are refreshed by the Lovable-owned project after migrations.
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let query = (supabaseAdmin as any).from("flight_schedule_records")
      ...
      .limit(20);
    const term = data.search ? safeFlightSearchTerm(data.search) : "";
    if (term) query = query.or(`flight_number.ilike.%${term}%,airline.ilike.%${term}%,origin.ilike.%${term}%,destination.ilike.%${term}%`);
    const { data: records, error } = await query;
    if (error) throw new Error(error.message);
    return (records ?? []) as LinkedFlightScheduleRecord[];
  });

/** Reads an already-linked immutable record, including one from an archived schedule. */
export const getLinkedFlightScheduleRecord = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  ...
  .handler(async ({ context }) => {
    ...
    const [activeFlightsResult, jobsResult, shipsResult, draftVersionsResult, shipEtaHistoryResult, shipEtaReviewCompletionsResult, companyResult, transportConflictReviewsResult, shipPortChangeReviewsResult, allShipJobsResult] = await Promise.all([
      ...
      tables
        ...
        .limit(10_000),
      tables
        .from("ship_eta_review_completions")
    ...
    const completedHistoryIds = new Set(
      ((shipEtaReviewCompletionsResult.data ?? []) as Array<{ eta_history_id: string }>).map((completion) => completion.eta_history_id),
    );
    ...
    const shipEtaChanges = ((shipEtaHistoryResult.data ?? []) as OperationsInboxShipEtaChange[])
      ...
      .filter((change) => !completedHistoryIds.has(change.id))
...
/**
 * Records completion of the current Ship ETA review after the coordinator has
 * manually reviewed the affected trips. It never changes a trip, ETA, or
 * pickup time; the immutable completion is an audit record only.
 */
export const completeShipEtaReview = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: unknown) => z.object({ eta_history_id: z.string().uuid() }).parse(input))
  .handler(async ({ data, context }) => {
    const c = await resolveCompany(context);
    const supabaseAdmin = await getAdminClient();
    ...
    const { data: history, error: historyError } = await tables
      .from("ship_event_eta_history")
      .select("id, ship_event_id, ship_events!inner(company_id)")
      .eq("id", data.eta_history_id)
      .eq("ship_events.company_id", c.id)
      .maybeSingle();
    if (historyError) throw new Error(historyError.message);
    if (!history) throw new Error("Ship ETA review not found.");

    const { data: latestChange, error: latestChangeError } = await tables
      .from("ship_event_eta_history")
      .select("id")
      .eq("ship_event_id", history.ship_event_id)
      .order("changed_at", { ascending: false })
      .limit(1)
      .maybeSingle();
    if (latestChangeError) throw new Error(latestChangeError.message);
    if (latestChange?.id !== history.id) {
      throw new Error("A newer Ship ETA change must be reviewed instead.");
    }

    const { count: linkedTripCount, error: linkedTripsError } = await tables
      .from("jobs")
      .select("id", { count: "exact", head: true })
      .eq("ship_event_id", history.ship_event_id)
      .gte("date", today)
      .or(jobScope);
    if (linkedTripsError) throw new Error(linkedTripsError.message);
    if (!linkedTripCount) throw new Error("This Ship ETA change has no linked operational trips to review.");

    const { error: completionError } = await tables.from("ship_eta_review_completions").insert({
...
// ---------- FLIGHT LIVE STATUS (AeroDataBox) ----------
// Flights resolve against AeroDataBox's real airline schedule data. There is
// no AI/search fallback: when a code can't be resolved we report that plainly
// rather than guessing, since a confidently-wrong arrival time is worse for a
// dispatcher than a clear "not tracked".
// Vessel live-status is switched off — see fetchLiveStatus.
// Results are cached in-memory for 20 minutes to avoid re-spending points on
// rapid refreshes.
...
// Master switch for the whole live-status feature.
...
async function fetchLiveStatusViaAeroDataBox(
  ...
  for (const provider of providers) {
    ...
    for (const date of datesToTry) {
      ...
      // AeroDataBox times are already proper timezone-tagged UTC ISO.
...
// A "no live status available" result — a successful, honest answer rather
// than an error, so callers persist a clean "not tracked" state instead of
// showing a broken/failed status card.
function notTracked(note: string): LiveStatusResult {
  return { ok: true, status: "unknown", note, scheduled: null, estimated: null, confidence: "low" };
}

// Dispatcher. Flights resolve via AeroDataBox (real airline schedule data);
// there is no fallback provider — when AeroDataBox can't resolve a code we
// say so plainly rather than guessing. Vessel live-status is switched off
// entirely: the only sources we had for it were best-effort at best, and a
// wrong ETA is worse than no ETA. Everything else about a vessel trip
// (driver location, route ETA, manual status) is unaffected.
async function fetchLiveStatus(
  ...
  if (kind === "vessel") return notTracked("Vessel live tracking is not available");
  if (!aeroDataBoxEnabled()) return notTracked("Live flight tracking not configured");
  if (looksLikeVessel(identifier)) return { ok: false, reason: "vessel_in_flight_field" };

  const r = await fetchLiveStatusViaAeroDataBox(identifier, pickupIso, side);
  if (r.ok) return r;
  if (r.reason === "invalid_code") return r;

  // Auto-fix: retry once with common trivial corrections (uppercase, strip
  // spaces, drop leading zeros) before giving up on the code.
  try {
    const { suggestCorrections } = await import("./flight-code");
    for (const alt of suggestCorrections(identifier)) {
      const retry = await fetchLiveStatusViaAeroDataBox(alt, pickupIso, side);
      if (retry.ok) {
...
// chip when the provider couldn't resolve a code. Applies a minimal patch to the
...
export const checkFlightStatus = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    for (const j of jobs) {
      ...
      // window — the provider cache would return the same answer and we'd burn
