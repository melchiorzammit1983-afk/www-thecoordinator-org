...
export const getClientTripPortal = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    const [{ data: siblings }, { data: pax }, { data: company }, { data: driver }] = await Promise.all([
      ...
      supabaseAdmin.from("companies").select("id, name, phone, coordinator_phone").eq("id", job.company_id).maybeSingle(),
      job.driver_id
    ...
    return {
      ...
      company: company ? {
        id: company.id,
        name: company.name,
        support_phone: (company as any).operations_phone ?? company.coordinator_phone ?? company.phone ?? null,
      } : null,
      driver,
      driverLocations,
      identity,
