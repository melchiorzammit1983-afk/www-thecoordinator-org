...
function CreatorBookingForm({ token, capabilities, bookingFields, portalName, recipientCompany }: { token: string; capabilities: BookingCapabilities; bookingFields: NormalizedPortalBookingFields; portalName: string; recipientCompany: string }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/public/portal/recipient/${token}/bookings`)
      ...
      .then((payload) => setOperationGroups((payload.groups ?? []).filter((group: OperationGroupOption) => ["draft", "active"].includes(group.status))))
  ...
  return <Card><CardHeader className="flex-row items-center justify-between gap-3"><div><CardTitle className="text-base">Create booking</CardTitle><p className="mt-1 text-xs text-muted-foreground">Enter one booking here or download the same fields as an Excel template.</p></div><Button size="sm" variant="outline" onClick={() => void downloadPortalBookingTemplate(portalName, bookingFields)}><Download className="mr-1 h-4 w-4" />Template (.xlsx)</Button></CardHeader><CardContent className="grid gap-4 sm:grid-cols-2">
    ...
    {capabilities.select_operation_group && isPortalFieldVisible(bookingFields, "operation_group") && <div className="space-y-1 sm:col-span-2"><Label>Operation Group{isPortalFieldRequired(bookingFields, "operation_group") ? " *" : ""}</Label><p className="text-xs text-muted-foreground">Only your Operations and Operations explicitly shared with you are available. Completed and cancelled Operations cannot accept new bookings.</p><select className="h-10 w-full rounded-md border bg-background px-3 text-sm" value={operationGroupId} onChange={(event) => setOperationGroupId(event.target.value)}><option value="">{isPortalFieldRequired(bookingFields, "operation_group") ? "Select an Operation Group" : "No Operation Group"}</option>{operationGroups.map((group) => <option key={group.id} value={group.id}>{group.reference} · {group.name} ({group.status})</option>)}</select></div>}
