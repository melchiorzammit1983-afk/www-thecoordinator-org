    ...
    <div className="min-h-screen bg-muted/30 pb-28">
      <header className="border-b bg-background px-4 py-4">
        <div className="max-w-3xl mx-auto flex items-center gap-3">
          <BrandLogo logoUrl={branding?.logo_url ?? null} name={branding?.company_name ?? data.link.subject_label ?? "C"} size="lg" />
          <div className="min-w-0">
            <div className="text-xs text-muted-foreground uppercase tracking-wide truncate">
              {branding?.company_name ?? "Your bookings"}
            </div>
            <h1 className="text-xl font-semibold truncate">{data.link.subject_label ?? "Client portal"}</h1>
          </div>
        </div>
      </header>
