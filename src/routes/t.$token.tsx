          ...
          <div className="min-w-0">
            ...
            {(job as any).route_duration_sec > 0 && (
              <div className="mt-1 text-[11px] opacity-80">
                ≈ {Math.max(1, Math.round((job as any).route_duration_sec / 60))} min
                {(job as any).route_distance_m ? ` · ${((job as any).route_distance_m / 1000).toFixed(1)} km` : ""} drive
              </div>
            )}
          </div>
        </div>
  ...
  return (
    <div className="p-4">
      <div className="grid grid-cols-3 gap-1 rounded-xl bg-slate-100 p-1 mb-3">
        ...
        >
          <Lock className="h-3.5 w-3.5" /> Coordinator
        </button>
        <button
          className={cn("rounded-lg py-2 text-xs font-medium flex items-center justify-center gap-1.5",
