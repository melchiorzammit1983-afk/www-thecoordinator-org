  ...
  server: {
    handlers: {
      GET: async ({ params, request }) => {
        ...
        if (visitorId && visitorId.length <= 80) {
          ...
          if (jobIds.length) {
            const { data: jrows } = await admin
              .from("jobs")
              .select(
                "id, status, pickup_at, from_location, to_location, driver_completed_at, vehicle, drivers(name, car_make_model, plate)",
              )
              .in("id", jobIds);
            jobs = jrows ?? [];
          }
        ...
        const past = (jobs as any[])
          .filter((j) => j.status === "completed" || j.status === "in_progress")
          .map((j) => ({
            id: j.id,
            when: j.driver_completed_at ?? j.pickup_at ?? null,
