...
CREATE OR REPLACE FUNCTION public.record_trip_audit(_job_id uuid, _event_type text, _previous jsonb DEFAULT NULL::jsonb, _new jsonb DEFAULT NULL::jsonb, _notes text DEFAULT NULL::text, _lat numeric DEFAULT NULL::numeric, _lng numeric DEFAULT NULL::numeric, _accuracy numeric DEFAULT NULL::numeric, _address text DEFAULT NULL::text, _speed numeric DEFAULT NULL::numeric, _device_time timestamp with time zone DEFAULT NULL::timestamp with time zone, _group_id uuid DEFAULT NULL::uuid, _stop_id uuid DEFAULT NULL::uuid, _approval_status text DEFAULT NULL::text, _driver_id uuid DEFAULT NULL::uuid, _actor_label text DEFAULT NULL::text)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  IF _actor IS NULL THEN
    RAISE EXCEPTION 'record_trip_audit: authentication required' USING ERRCODE = '42501';
  END IF;
  ...
  -- Authorization: admin, company owner, or the assigned driver.
  IF private.is_admin(_actor) THEN
    _authorized := true;
  ELSE
    SELECT owner_user_id INTO _company_owner FROM public.companies WHERE id = _company_id;
    IF _company_owner = _actor THEN
      _authorized := true;
    ELSIF _job_driver IS NOT NULL AND EXISTS (
      SELECT 1 FROM public.drivers d
      WHERE d.id = _job_driver AND d.linked_user_id = _actor
    ) THEN
      _authorized := true;
    END IF;
  END IF;
  ...
  IF _label IS NULL THEN
    IF private.is_admin(_actor) THEN _label := 'admin';
    ELSIF _company_owner = _actor THEN _label := 'coordinator';
    ELSE _label := 'driver';
    END IF;
  END IF;
