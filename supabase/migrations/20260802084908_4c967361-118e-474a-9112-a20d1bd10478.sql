-- ==== 20260724153000_transport_core_rebuild.sql ====
do $$
begin
...
create table if not exists public.trips (
  ...
  parent_trip_id uuid references public.trips(id) on delete set null deferrable initially deferred,
  source text not null default 'manual',
...
select
  ...
  (case when j.status in ('completed', 'cancelled') then 'completed' else 'active' end)::public.operation_status,
  'legacy_job',
  j.created_at,
  j.updated_at
from public.jobs j
...
update public.pax p
   set operation_id = j.operation_id
  from public.jobs j
 where j.id = p.job_id
   and p.operation_id is distinct from j.operation_id;

alter table public.pax
  alter column operation_id set not null;

-- ==== 20260725160000_fix_pax_operation_link.sql ====
create or replace function public.mirror_pax_to_transport_core()
returns trigger
language plpgsql
security definer
...
-- ==== 20260801160000_trip_flight_schedule_link.sql ====
ALTER TABLE public.jobs
...
-- ==== 20260802061035_operational_pickup_offsets.sql ====
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
     WHERE table_schema = 'public' AND table_name = 'companies'
       AND column_name = 'default_departure_pickup_offset_minutes'
  ) THEN
    ALTER TABLE public.companies
      ADD COLUMN default_departure_pickup_offset_minutes integer NOT NULL DEFAULT 180
        CHECK (default_departure_pickup_offset_minutes BETWEEN 0 AND 1440);
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
     WHERE table_schema = 'public' AND table_name = 'companies'
       AND column_name = 'default_arrival_pickup_offset_minutes'
  ) THEN
    ALTER TABLE public.companies
      ADD COLUMN default_arrival_pickup_offset_minutes integer NOT NULL DEFAULT 0
        CHECK (default_arrival_pickup_offset_minutes BETWEEN 0 AND 1440);
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
     WHERE table_schema = 'public' AND table_name = 'jobs'
       AND column_name = 'scheduled_transport_pickup_offset_minutes'
  ) THEN
    ALTER TABLE public.jobs
      ADD COLUMN scheduled_transport_pickup_offset_minutes integer
