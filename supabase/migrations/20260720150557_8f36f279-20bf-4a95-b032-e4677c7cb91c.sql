...
BEGIN
  ...
  IF _is_ai THEN
    ...
    IF _remaining > 0 AND FOUND AND _sub.points_remaining_this_period > 0 THEN
      _take := LEAST(_remaining, _sub.points_remaining_this_period);
      UPDATE public.company_subscriptions
        SET points_remaining_this_period = points_remaining_this_period - _take
        WHERE id = _sub.id;
      _remaining := _remaining - _take;
    END IF;
