-- Crew Management System — Phase 1: core HR crew sheet
-- Crew members are reusable profiles scoped to a client "portal_companies" row
-- (the existing hotel/agent/corporate portal at /portal/$token). Itinerary legs
-- are stored separately so a crew member can be reused across contracts.
...
-- One active (non-deleted) crew profile per email per portal company.
CREATE UNIQUE INDEX IF NOT EXISTS idx_crew_members_portal_email_active
  ON public.crew_members(portal_company_id, lower(email))
  WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_crew_members_portal ON public.crew_members(portal_company_id);
CREATE INDEX IF NOT EXISTS idx_crew_members_link_token ON public.crew_members(link_token);
...
-- Itinerary legs (up to 3 per crew member): reusable profile, per-trip legs.
CREATE TABLE IF NOT EXISTS public.crew_itineraries (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS idx_crew_itineraries_crew_member ON public.crew_itineraries(crew_member_id);
